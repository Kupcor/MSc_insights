...
def train_model(model, X_train, y_train, loss_fun, opt_func, epochs):
    ...
    #scheduler = ls.ReduceLROnPlateau(opt_func, mode='min', factor=0.8, patience=120, verbose=True)
    ...
    print(f'Training finished. Train time: {train_time} | R2 error: {r2}')
...
def train_model_with_batches(model, X_train, y_train, loss_fun, opt_func, epochs):
    #   Additional data gathering
    losses_for_training_curve = []
    ...
    scheduler = ls.StepLR(opt_func, step_size=hp.num_epochs//5, gamma=0.1)   #LS scheduler -> TODO parametrized and improve it   
    ...
    for epoch in range(epochs + 1):
        ...
        for batch in train_loader:
            ...
            train_loss = loss_fun(predictions, targets.unsqueeze(1))
            train_loss.backward()
            opt_func.step()

            average_test_loss += train_loss.item()

        average_test_loss = average_test_loss / len(train_loader)       
        if epoch % 100 == 0 or epoch == epochs:
            print(f'Epoch num: {epoch} / {epochs} completed | Loss for this epoch: {average_test_loss} | LR: {round(opt_func.param_groups[0]["lr"], 7)} | Optimizer: {opt_func.__class__.__name__} | Device: {next(model.parameters()).device.type}')
        scheduler.step()
        ...
        if abs(train_loss.item() - previous_train_loss) < 0.00001:
            epoch_till_not_change+=1
        else:
            epoch_till_not_change = 0
