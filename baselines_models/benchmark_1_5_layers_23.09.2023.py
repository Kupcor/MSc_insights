...
def train_model(train_loader, model, loss_fun, opt_func, epochs):
    train_model_file_name = f"_p_{hp.lr}_{hp.num_epochs}_{opt_func.__class__.__name__}_{today}"
...
def test_model(test_loader, model,loss_fun,show_results=True, opt="Adam"):
    test_model_file_name = f"_p_{hp.lr}_{hp.num_epochs}_{opt.__class__.__name__}_{today}"
...
def run_loaded_model(file_path,X_val=X_test, Y_val=y_test, loss_fun = nn.MSELoss()):
    ...
    model.eval()
    predictions, reall_values, test_loss = test_model(test_loader=test_loader, model=model, loss_fun=nn.MSELoss(), opt=hp.optimizer_arg, show_results=True)
